<img src="https://github.com/DaoChuanXS/WearNextBook/blob/main/res/logo.png" width="100px"/></p>
<h1 >腕上教材</h1>

`Created by Atomic UI 1.0 , Designed by Irregular`

`Copyright © 2022-2023 Irregular. All Rights Reserved.`

------------------------------------
<p>
<div align=center>
<img src="https://raw.githubusercontent.com/Irregular2333/WearElements/main/Readme%20Files/Start.jpg" width="400px" >
</p>
<p>

`2022.10.28 - 2023.10.28`

`腕上周期表/WearElements 一周年`
</div>
</p>

## 项目介绍

### 应用简介

一款可以在安卓智能手表和wearOS手表上面运行的化学学习类软件，除了最基本的元素信息查询，还支持离子，有机基团等多种数据查询

### 项目截图

![预览图片](https://raw.githubusercontent.com/Irregular2333/WearElements/main/Readme%20Files/screenshots.png)



### 如何安装

- [加入官方交流群：672510064（推荐）](http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=8CGhOcmjRMm683W5ZYUPtjfM4hr2BsfQ&authKey=vMPvH7UvN9sGr8Nw5lcXta4OVclGIrbSc8V5TbwvlF0WgS9R8ZEY698SVZx8OPj6&noverify=0&group_code=672510064)
- [Github Releases](https://github.com/Irregular2333/WearElements/releases/)
- [蓝奏云(大概率链接更新不及时)](https://bmslm.lanzoul.com/ik1VT1d24cgj)
- 唯趣应用商店、洋葱应用商店（推荐）等第三方应用商店
> 建议前往哔哩哔哩，酷安等平台搜索将apk安装至手表的方式


## 宣传视频

[《耗时三个月，我做了一款手表上的元素周期表》](https://www.bilibili.com/video/BV1mu4y117Xf/)



## 联系作者 & 捐赠

- [哔哩哔哩：@Irregularly](https://space.bilibili.com/390148599)
- [官方交流群：672510064](http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=8CGhOcmjRMm683W5ZYUPtjfM4hr2BsfQ&authKey=vMPvH7UvN9sGr8Nw5lcXta4OVclGIrbSc8V5TbwvlF0WgS9R8ZEY698SVZx8OPj6&noverify=0&group_code=672510064)

> 腕上周期表/WearElements使用iApp编写，是完全开源且免费的。与此同时，维护软件的更新和更新服务器需要投入资金和大量的精力，您可以选择以下的方式支持我们，感谢所有人的支持

<p>
<div style="display: flex; justify-content: center;">
<img src="https://raw.githubusercontent.com/Irregular2333/WearElements/main/Readme%20Files/Wechat.png" width="240px"/>
</p>
<p>
<img src="https://raw.githubusercontent.com/Irregular2333/WearElements/main/Readme%20Files/AliPay.jpg" width="200px"/>
</div>
</p>

## Licence

本项目在 AGPL-3.0 许可证下开源，详情参阅[LICENSE](https://github.com/Irregular2333/WearElements/blob/main/LICENSE)文件。

**但请注意，您必须同时遵守下面的额外限制：**
<br>
**However, please note that you must also observe the following additional restrictions:**

- 基于（修改）本项目所开发的新项目，必须保留本项目对包括但不限于特定设备、地域、人群的限制，包括源代码和二进制都必须遵守此限制。
- Based on (modifying) the new project developed by this project must retain the limitations of this project including but not limited to specific devices, regions, and crowds, including source code and binary must comply with this restriction.

- 基于（修改）本项目所开发的新项目，其许可证必须包含所有这里列出的额外限制条件。
- Based on (modifying) the new project developed by this project, the license must include all additional restrictions listed here.

## 写在最后
关于这个项目，其实一直有些话想说，就写在这里了

<p>
<div align=center>
<img src="https://raw.githubusercontent.com/Irregular2333/WearElements/main/Readme%20Files/Memory.jpg" width=600px" >
</p>
<p>

`谨以此项目记念我的初中时光 那是一段我再也回不去、短暂的但又美好的时光`
</div>
</p>

### 立项的原因？

这个项目是去年(2022年)十月底立项的，最开始的时候其实是想方便自己的学习罢。我在去年的8月份购入了人生中的第一块智能手表：OPPO Watch 2，在11月购入了第二块智能手表：OPPO Watch 3Pro，这才有了后面的一切。

最开始我也就是一名普通的智能手表用户，~~毕竟是学生，所以看看视频刷刷酷安很正常好吧~~，但是随着对智能手表的深入了解，我逐渐意识到，安卓智能手表的生态真的是非常贫瘠（和隔壁AppleWatch相比）；

这也是为什么，穿戴圈的个人开发者如此之多的原因之一，~~要是安卓智能手表生态好软件足够好用谁他妈自己做软件~~，所以说在受到一天晚自习的启示之后，我在那周周末回家之后就立下了这个项目


### 为什么而做？

很简单，算是自己的热爱吧，~~虽然我真的很菜~~

这个项目是使用iApp编写的，在这里要感谢冬日，是他带我入坑了这门语言，也是他给了我很多帮助，真的很感谢感谢冬日喵~

这个项目刚立项的时候就确定了`全免费`，算是为爱发电吧，因为我那个时候还有大把资本可以拿来为爱发电

### 这一年里经历了什么？

这里面的故事很长很长喽，如果什么时候有时间再来慢慢说吧


`永远相信美好的事情即将发生`

**在README.md的最后，我想放一些我最喜欢的英文歌`《I Really Want To Stay At Your House》`的歌词：**

`I couldn't wait for you to come clear the cupboards.`

`But now you're going to leave with nothing but a sign.`

`Another evening I'll be sitting reading in between your lines.`

`Because I miss you all the time.`

**还想补充另外一首歌`《那些花儿》`，这首歌比我的年龄都大。曾经理解不了那些怀念从前的人，当我反应过来时，已经深陷其中了**

`今天我们已经离去 在人海茫茫`

`他们都老了吗`

`他们在哪里啊`

`幸运的是我 曾陪他们开放`

**我不知道未来是什么样，但我希望我们都能拥有一个光明的未来**

**感谢您能看到这里，谢谢**
